...
edge_all<-read.csv("MSPPIN_edge.csv",header = T,sep = ",")
net_all<- make_graph(t(edge_all),directed = FBLSE)
...
groupA_avesp<-data.frame(symbol=c(NA),num_V_newnet=c(NA),sum_SPL=c(NA),sum_Inf=c(NA),num_neighbor_in_B=c(NA),PS-V2N=c(NA))
i=0
for (node in groupA) {
  i=i+1
...
groupA_avesp<-groupA_avesp[order(groupA_avesp$PS-V2N),]
order_by_PS-V2N<-c(1:i)
groupA_avesp<-cbind(order_by_dV2N,groupA_avesp)

write.csv(groupA_avesp,"groupA_avesp.csv",row.names = F)
